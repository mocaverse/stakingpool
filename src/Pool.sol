...
//-------------------------------external-------------------------------------------
    ...
    function updateNftFee(bytes32 vaultId, address onBehalfOf, uint256 newNftFeeFactor) external whenStarted whenNotPaused auth {
       ...
       (DataTypes.UserInfo memory userInfo, DataTypes.Vault memory vault) = _updateUserIndexes(onBehalfOf, userInfo_, vault_);
        ...
        if(newNftFeeFactor <= vault.accounting.nftFeeFactor) revert Errors.NftFeeCanOnlyBeIncreased(vaultId);
        ...
        emit NftFeeFactorUpdated(vaultId, vault.accounting.nftFeeFactor, newNftFeeFactor);
        
        // update Fee factor
        vault.accounting.nftFeeFactor = newNftFeeFactor;
